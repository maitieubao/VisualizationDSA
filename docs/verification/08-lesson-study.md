...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 9/10 🟢 Thực dụng (tăng từ 8/10 — nội dung 4 bước có thật)
...
| :-- | :-- | :-- |
| LessonStudyView (4 bước + gating premium + StepTabs khóa bước) | `frontend/src/views/lesson/LessonStudyView.vue` + `features/lesson/components/StepTabs.vue` | ✅ |
| Codelab thật ở bước 4 (resolve codelabId → payload API → codelabTask; fallback registry demo) | `frontend/src/features/lesson/store/useLessonStore.ts` (buildLessonFromApi) | ✅ |
| Curriculum teacher (module/item CRUD, reorder, override, import) | `backend/src/Application/Features/Classrooms/*` + FE teacher tab | ✅ |
| Student curriculum sidebar (khóa/mở theo prerequisite, deep-link `?itemId`) | `frontend/src/features/classroom/*` | ✅ |
| Quiz liên kết lesson (heuristic "quiz ngay sau lesson") | `LessonController.GetLessonById` | ✅ |
| Nội dung 4 bước có thật (A2): 40 lesson seed đủ lý thuyết + sandbox + quiz + codelab (5 bài codelab DB) | `DbSeeder.cs` | ✅ NEW |
