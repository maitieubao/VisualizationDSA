...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 5/10 🟡 Demo-grade (tăng từ 3/10 nhờ C3 docs)
...
| :-- | :-- | :-- |
| EmbedCommunicationBridge (postMessage, origin whitelist fail-closed, shape validate) | `frontend/src/features/embed-widget/engine/EmbedCommunicationBridge.ts` | ✅ |
| AutoHeightResizer (ResizeObserver + debounce 100ms + clamp 300-1200px) | `engine/AutoHeightResizer.ts` | ✅ |
| SecureOriginChecker (allowlist) | `engine/SecureOriginChecker.ts` | ✅ |
| Configurator (theme/algo/dimensions/VCR, sinh mã iframe, copy) | `store/useEmbedConfiguratorStore.ts` + `EmbedConfiguratorSidebar.vue` | ✅ |
| EmbedCodeSnippet (iframe code + host integration script có verify source/origin) | `components/EmbedCodeSnippet.vue` | ✅ |
| LiveWidgetPreview (iframe thật, loading/error state) | `components/LiveWidgetPreview.vue` | ✅ |
| **C3: `docs/host/HOST_GUIDE.md`** — tài liệu host hoàn chỉnh (dán snippet 3 cách, host script, điều khiển, allowlist, xử lý lỗi, checklist) | `docs/host/HOST_GUIDE.md` | ✅ NEW |
| **C3: `docs/host/sample-host.html`** — trang demo host + nút điều khiển + log cross-origin | `docs/host/sample-host.html` | ✅ NEW |
| Route `/embed` | `frontend/src/router/routes.ts` | ✅ |
...
- **⚠️ CHƯA verify trên LMS thật** (Moodle/Canvas) — toàn bộ bằng chứng là test + tự nhúng.
