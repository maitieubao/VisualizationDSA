...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 8/10 🟢 Thực dụng (tăng từ 5/10 — hết "bell rỗng")
...
| :-- | :-- | :-- |
| NotificationService (tạo + push realtime broker + hub `Clients.User`, chống spoof) | `backend/src/Infrastructure/Services/NotificationService.cs` + `NotificationHub.cs` | ✅ |
| **Nguồn 1: comment reply** (có sẵn) | `LessonController.CreateLessonComment` → NotifyUserAsync parent | ✅ |
| **Nguồn 2: level-up** — `GamificationService.AwardXpAndCheckBadgesAsync` + `LessonController.CompleteLesson` gọi `NotifyLevelUpAsync` SAU commit (lỗi không phá request) | `GamificationService.NotifyLevelUpAndBadgesAsync` + `CompleteLesson` | ✅ NEW |
| **Nguồn 3: badge award** — `NotifyBadgeAwardedAsync` cho từng badge mới | `GamificationService` | ✅ NEW |
| **Nguồn 4: bài mới trong lớp** — CreateClassroomModuleItem notify học viên ACTIVE (bỏ item ẩn + bị kick) | `CreateClassroomModuleItemCommandHandler.cs` | ✅ NEW |
| **Nguồn 5: deadline lớp** — DeadlineReminderService (BackgroundService mỗi giờ, DueAt trong 24h chưa hoàn thành → nhắc, dedupe ngày) | `backend/src/Infrastructure/Services/DeadlineReminderService.cs` | ✅ NEW |
| FE: connect sau login + polling 60s backup, toast + bell + dedupe, 401 auto-refresh, unread-count đúng >100, a11y | `frontend/src/features/notifications/*` | ✅ |
