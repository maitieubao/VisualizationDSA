...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 9/10 🟢 Thực dụng (giữ nguyên — không thay đổi trong Phase A→D)
...
| :-- | :-- | :-- |
| Stateless Auth (JWT access + refresh, rotation chống reuse, single-flight refresh) | `backend/src/Domain/Strategies/StatelessAuthStrategy.cs` | ✅ |
| Stateless Auth API (login/register/refresh/logout) | `backend/src/WebApi/Controllers/StatelessAuthController.cs` | ✅ |
| AuthService (classic user/pass, hash SHA-256) | `backend/src/Application/Services/AuthService.cs` | ✅ |
| JWT signing config (fail-closed — thiếu key tự throw) | `backend/src/Domain/JwtSigningConfig.cs` | ✅ |
| Auth store FE (login/register/logout/refresh, 401 auto-refresh) | `frontend/src/features/auth/store/useAuthStore.ts` | ✅ |
| Router guard (requiresAuth / requiresRole / impersonation) | `frontend/src/router/index.ts` + `routes.ts` | ✅ |
| Impersonate (admin đóng vai student, banner + thoát về /admin) | `backend/src/WebApi/Controllers/AdminController.cs` + FE | ✅ |
| Refresh token cleanup (xóa token hết hạn sau 7 ngày) | `backend/src/Infrastructure/Services/RefreshTokenCleanupService.cs` | ✅ |
...
## 4. 🖥️ Các bước xác thực thủ công (bạn tự chạy)
