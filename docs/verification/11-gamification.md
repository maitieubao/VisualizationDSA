...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 8/10 🟢 Thực dụng (tăng từ 7/10)
...
| :-- | :-- | :-- |
| XP server-side: idempotency (user+ngày+key), cap 500 XP/ngày, rate limit, không tin client | `backend/src/Infrastructure/Services/GamificationService.cs` (XpGrantLedger) | ✅ |
| **A2/A3: nguồn XP THẬT từ học bài** — CompleteLesson cấp XP mỗi bài (40 bài seed), E2E verify idempotent | `LessonController.CompleteLesson` | ✅ NEW |
| Badge: 1 nguồn id, criteria thật, unique (UserId, BadgeId) chặn race | `GamificationService.CheckAndAwardBadgesAsync` | ✅ |
| **C2: notification level-up + badge award** — gọi `NotifyLevelUpAsync`/`NotifyBadgeAwardedAsync` SAU commit (lỗi không phá request) | `GamificationService.NotifyLevelUpAndBadgesAsync` + `CompleteLesson` | ✅ NEW |
| Streak server source of truth (timezone-safe, freeze 1 ngày) | `StreakCalculator` + User entity | ✅ |
| Leaderboard realtime (broker publish sau commit + hub authorize) | `LeaderboardBroadcastBroker` + `LeaderboardHub` | ✅ |
| FE: Workspace gamification (XP/streak/badge cabinet/confetti) | `frontend/src/features/gamification-engine/*` | ✅ |
| Nút "+50 XP (Demo)" — chỉ Teacher/Admin + nhãn Demo + title giải thích (D1) | `GamificationWorkspace.vue` | ✅ NEW |
