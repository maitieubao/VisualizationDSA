...
| :-- | :-- | :-- |
| Store playground (code, auto-run debounce, lỗi compile, persist) | `frontend/src/features/html-playground/store/useHtmlPlaygroundStore.ts` | ✅ |
| Preview render (iframe sandbox `allow-scripts allow-forms`) | `frontend/src/features/html-playground/components/PlaygroundPreview.vue` | ✅ |
| Workspace tổng (editor + preview + VCR) | `PlaygroundWorkspace.vue` | ✅ |
| Document builder (ghép HTML/CSS/JS thành 1 document) | `engine/PlaygroundDocumentBuilder.ts` | ✅ |
| Debouncer (tự chạy lại sau khi dừng gõ) | `engine/PlaygroundDebouncer.ts` | ✅ |
| URL codec (chia sẻ code qua URL) | `engine/PlaygroundUrlCodec.ts` | ✅ |
| Bộ demo mẫu | `demos/playgroundDemos.ts` | ✅ |
| Route `/playground` (mode HTML) | `frontend/src/router/routes.ts` | ✅ |
