...
| :-- | :-- | :-- |
| TeacherPanelView + tabs (Quiz, Codelab, Course, Lớp, Học viên, Analytics, Theory) | `frontend/src/views/teacher/TeacherPanelView.vue` | [X] |
| **TeacherCourseTab nâng cấp (A1.4)** — tab Soạn thảo/Xem trước markdown an toàn, codelab picker, sandboxConfig JSON validate, publish status (Draft/Private/Published), nút "Xem trước như học viên" | `frontend/src/views/teacher/TeacherCourseTab.vue` | [X] MOI |
| Quiz Builder full CRUD (question form, picker, import từ ngân hàng) | `QuizBuilderTab.vue` + `useQuizBuilder.ts` + modals | [X] |
| Codelab Builder full CRUD (template/hint/testcase lưu thật) | `CodelabBuilderTab.vue` + modals | [X] |
| Theory Article Library + editor | `TheoryArticleLibraryTab.vue` + `TheoryArticleEditorModal.vue` | [X] |
| Analytics tab (completionRate x100, Export Excel có loading) | `TeacherAnalyticsTab.vue` + `TeacherClassroomAnalytics.vue` | [X] |
| API teacher chuẩn (401 retry, error banner) | `useTeacherApi.ts` | [X] |
