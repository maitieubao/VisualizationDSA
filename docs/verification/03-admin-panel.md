# 🛠️ Báo Cáo Xác Thực — 03. Admin Panel
...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 9/10 🟢 Thực dụng
...
| :-- | :-- | :-- |
| 13+ endpoint quản trị (dashboard, users CRUD, role, premium, ban, reset-password, quizzes, impersonate, audit-logs, **analytics/learning**) | `backend/src/WebApi/Controllers/AdminController.cs` | ✅ |
| **D4: `GET /api/v1/concepts/admin/analytics/learning`** — per-lesson stats + overall + **tương quan "xem viz → pass quiz"** | `AdminController.GetLearningAnalytics` | ✅ NEW |
| AdminPanelView + 6 tab (Tổng quan, Người dùng, Quản lý Quiz, **Học tập**, Hệ thống, Nhật ký) | `frontend/src/views/admin/AdminPanelView.vue` | ✅ |
| Tab "Học tập" — 5 card tổng quan + biểu đồ so sánh viz→quiz + bảng chi tiết bài | `frontend/src/views/admin/AdminLearningTab.vue` | ✅ NEW |
| AdminRequest chuẩn (timeout 15s + 401 auto-refresh retry) | `frontend/src/views/admin/useAdminApi.ts` | ✅ |
| Dashboard + System: số liệu thật + fallback `isFallback` khi DB down | `AdminDashboardTab.vue` + `AdminSystemTab.vue` | ✅ |
| A11y tabs (tablist, ArrowLeft/Right), focus trap modal, lưu tab vào query | `AdminPanelView.vue` | ✅ |
