...
| :-- | :-- | :-- |
| ProfileView + 6 tab (General, Progress, History, Preferences, Security, About) | `frontend/src/views/profile/ProfileView.vue` + tabs | [X] |
| UpdateProfile persist (username/nickname/bio/university/avatar) | backend `UsersController` + `User.UpdateProfile` | [X] |
| Tiến trình XP + tủ badges dữ liệu thật | `ProfileProgressTab.vue` | [X] |
| Lịch sử học (bank quiz attempt...) | `ProfileHistoryTab.vue` | [X] |
| Preferences (tùy chọn học) | `ProfilePreferencesTab.vue` | [X] |
| Đổi mật khẩu + bảo mật | `ProfileSecurityTab.vue` | [X] |
| Route `/profile` | `frontend/src/router/routes.ts` | [X] |
