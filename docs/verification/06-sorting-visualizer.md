...
| :-- | :-- | :-- |
| Algorithm Dashboard (danh mục + chọn thuật toán) | `frontend/src/features/dsa-modules/components/AlgorithmDashboard.vue` | ✅ |
| AlgorithmVisualizer (dynamic `<component :is>` theo render mode) | `AlgorithmVisualizer.vue` | ✅ |
| DSAPlayer (VCR điều khiển) + DSAHeader + Legend + PseudocodeViewer | `DSAPlayer.vue`, `DSAHeader.vue`, `Legend.vue`, `PseudocodeViewer.vue` | ✅ |
| 4 renderer: BarChart, BoxArray, Tree, Tube + helpers | `components/renderers/*` | ✅ |
| Bộ sinh dữ liệu + generators theo thuật toán (sorting/searching/premium) | `services/*Generators.ts` + `algorithmCatalog.ts` | ✅ |
| Store orchestration + phím tắt | `useAlgorithmStore.ts` + `useDSAKeyboard.ts` | ✅ |
| Route `/sorting` | `frontend/src/router/routes.ts` | ✅ |
| Backend: engine thuật toán C# (Strategy Pattern + Reflection DI) | `backend/src/Domain/Strategies/*` + `AlgorithmsController.cs` | ✅ |
