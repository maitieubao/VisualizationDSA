...
> **Ngày báo cáo:** 2026-08-14 · **Điểm giá trị thực tế hiện tại:** 7/10 🟡 Demo-grade (tăng từ 6/10 nhờ C4)
...
| :-- | :-- | :-- |
| SVGToCanvasExporter (extractSVGDataURI, clampScale 1-4, exportToPNG 3x, exportToSVGString) | `frontend/src/features/export-share/engine/SVGToCanvasExporter.ts` | ✅ |
| WorkspaceStateCompressor (lz-string, validate ≤20000) + QR (giới hạn 2500) | `engine/WorkspaceStateCompressor.ts` | ✅ |
| ExportShareView (modal export PNG/SVG + share link + QR + copy) | `frontend/src/views/export-share/ExportShareView.vue` | ✅ |
| ShareRestoreView (`/s/` restore snapshot, error state) | `frontend/src/views/export-share/ShareRestoreView.vue` | ✅ |
| **C4: "Xuất ảnh PNG" trong Algo Playground** — canvas.toDataURL + tên `visualization-{demo}-step-{n}.png` | `AlgoPlaygroundWorkspace.vue` (onExportPng) | ✅ NEW |
| Route `/export-share` + `/s` | `frontend/src/router/routes.ts` | ✅ |
