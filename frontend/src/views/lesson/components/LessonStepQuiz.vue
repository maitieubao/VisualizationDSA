<template>
  <div class="lesson-step-quiz flex flex-col h-full overflow-y-auto p-6 text-slate-200 font-sans max-w-3xl mx-auto w-full">
    ...
    <div v-if="questions.length > 0" class="flex flex-col gap-6 flex-1">
      ...
      <div class="mt-4 p-5 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-between">
        ...
        <button
          ...
          class="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl text-xs font-bold transition-all shadow-lg cursor-pointer"
        >
    ...
    <div v-else class="flex flex-col items-center justify-center flex-1 text-center py-12">
      ...
      <p class="text-xs text-slate-400 mt-1 max-w-md">Bấm bên dưới để xác nhận bạn đã nắm vững kiến thức và tiến tới phần Code Lab.</p>
      <button
        @click="$emit('completeStep')"
        class="mt-6 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg cursor-pointer"
      >
        Mở Khóa Code Lab →
      </button>
    </div>
