<template>
  <div class="h-full overflow-hidden">
    <DocsLayout>
      <template #default>
        <CheatSheetView v-if="isCheatSheet" />
        <DocsMarkdownRenderer 
          v-else-if="currentDocRaw" 
        ...
        <div v-else-if="!loading" class="flex flex-col items-center justify-center py-20 text-center">
          <div class="w-24 h-24 mb-6 text-text-muted">
          ...
          <router-link to="/docs/intro" class="mt-6 px-4 py-2 bg-accent-primary text-text-primary rounded-md hover:bg-accent-secondary transition-colors">
...
const loadMarkdown = async () => {
  ...
  if (subPath === 'cheatsheet') {
    isCheatSheet.value = true;
    loading.value = false;
    return;
  }
