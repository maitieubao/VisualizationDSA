      ...
      <div v-else-if="course" class="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div class="lg:col-span-8 flex flex-col gap-8">
          ...
          <section class="mt-8">
            ...
            <!-- Timeline Container -->
            <div class="relative pl-6 md:pl-10 border-l-2 border-slate-700/50 space-y-8 ml-2 md:ml-4">
              ...
              >
                ...
                <div 
                  class="absolute -left-[35px] md:-left-[51px] top-6 w-6 h-6 rounded-full border-4 border-slate-950 flex items-center justify-center transition-all duration-500 z-10"
                  :class="[
                    lesson.status === 'Completed' 
                      ? 'bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.7)]' 
                      : (lesson.status === 'InProgress' || (idx === 0 && lesson.status === 'NotStarted'))
                        ? 'bg-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.7)] animate-pulse'
                        : 'bg-slate-700 border-slate-950 shadow-inner'
                  ]"
                >
                  <span v-if="lesson.status === 'Completed'" class="text-slate-950 text-[10px] font-black">✓</span>
                </div>
