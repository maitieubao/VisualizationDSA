      ...
      <div v-if="showCreateUserModal" class="user-modal-backdrop" @click.self="closeCreateUserModal">
        <div class="user-modal-card">
          ...
          <form @submit.prevent="submitCreateUser" class="modal-form mt-4">
            <div class="form-group mb-4"><label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">Username</label><input v-model="createUserForm.username" type="text" class="form-control w-full" placeholder="Nhập username..." required /></div>
            <div class="form-group mb-4"><label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">Email</label><input v-model="createUserForm.email" type="email" class="form-control w-full" placeholder="example@visualizationdsa.dev" required /></div>
            <div class="form-group mb-4"><label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">Mật khẩu ban đầu</label><input v-model="createUserForm.password" type="password" class="form-control w-full" placeholder="Tối thiểu 8 ký tự..." required /></div>
            <div class="form-group mb-4"><label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">Vai trò (Role)</label><select v-model="createUserForm.role" class="form-control w-full"><option value="Student">Học viên</option><option value="Teacher">Giảng viên</option><option value="Admin">Quản trị viên</option></select></div>
            <div class="form-group mb-6 flex items-center gap-2"><input v-model="createUserForm.isPremium" type="checkbox" class="w-4 h-4 rounded border-border-default bg-bg-primary/40 text-accent focus:ring-accent" /><label class="text-xs font-bold text-text-secondary select-none cursor-pointer">Kích hoạt tài khoản Premium</label></div>
  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div v-if="showResetPasswordModal" class="user-modal-backdrop" @click.self="closeResetPasswordModal">
        <div class="user-modal-card">
          <div class="user-modal-header">
            <div><h2 class="user-modal-name text-text-primary">Đặt lại mật khẩu</h2><p class="text-xs text-text-secondary mt-1">Đổi mật khẩu cho: {{ targetUserForReset?.username }}</p></div>
          ...
          <form @submit.prevent="submitResetPassword" class="modal-form mt-4">
            <div class="form-group mb-6"><label class="block text-xs font-bold text-text-secondary mb-1.5 uppercase">Mật khẩu mới</label><input v-model="resetPasswordForm.password" type="password" class="form-control w-full" placeholder="Tối thiểu 8 ký tự..." required /></div>
