<template>
  <div class="teacher-classroom-analytics min-h-screen bg-bg-primary p-6">
    <header class="mb-8">
      <button type="button" class="text-text-secondary hover:text-text-primary transition-colors mb-4 flex items-center gap-2" @click="$router.back()">
      ...
      <p class="text-text-secondary mt-1">Thống kê tiến độ và kết quả học tập</p>
    </header>

    
    <div v-if="loading" class="flex items-center justify-center py-20">
      <div class="w-10 h-10 border-4 border-border-accent border-t-transparent rounded-full animate-spin"></div>
    </div>

    <template v-else-if="stats">
      
      <div class="grid grid-cols-4 gap-4 mb-8">
        <div class="bg-bg-secondary/60 border border-border-default rounded-2xl p-5">
          <div class="text-sm text-text-secondary mb-1">Tổng học viên</div>
          <div class="text-3xl font-bold text-text-primary">{{ stats.totalStudents }}</div>
        </div>
        <div class="bg-bg-secondary/60 border border-border-default rounded-2xl p-5">
          <div class="text-sm text-text-secondary mb-1">Điểm trung bình</div>
          <div class="text-3xl font-bold text-accent-cyan">{{ stats.avgScore }}%</div>
        </div>
        <div class="bg-bg-secondary/60 border border-border-default rounded-2xl p-5">
          <div class="text-sm text-text-secondary mb-1">Tỷ lệ đạt</div>
          <div class="text-3xl font-bold text-accent-green">{{ stats.passRate }}%</div>
        </div>
        <div class="bg-bg-secondary/60 border border-border-default rounded-2xl p-5">
          <div class="text-sm text-text-secondary mb-1">Hoàn thành</div>
          <div class="text-3xl font-bold text-accent-warm">{{ (stats.completionRate * 100).toFixed(1) }}%</div>
        </div>
      </div>

      
      <div class="bg-bg-secondary/60 border border-border-default rounded-2xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="student in stats.studentScores" :key="student.studentId" class="border-b border-border-default hover:bg-bg-surface">
                ...
                <td class="text-center p-4 font-mono text-accent-warm">{{ student.totalXP }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function getScoreClass(score: number | undefined | null): string {
  ...
  if (score >= 50) return 'text-accent-warm';
