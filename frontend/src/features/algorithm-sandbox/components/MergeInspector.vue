  ...
  <div class="merge-panel border rounded-2xl p-4 backdrop-blur-md flex flex-col gap-3 h-full">
    <div class="flex items-center justify-between border-b border-border-subtle pb-2 shrink-0">
      ...
      <span 
        ...
        :class="activeSub ? 'bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20 animate-pulse' : 'bg-slate-500/10 text-text-muted border border-border-subtle'"
      >
    ...
    <div v-if="activeSub && frame" class="flex-1 flex flex-col gap-4 justify-between">
      ...
      <div class="flex-1 flex flex-col justify-between p-3 rounded-xl merge-inner-block min-h-0 text-xs">
        <div class="flex flex-col gap-2">
          ...
          <div class="grid grid-cols-3 gap-2 text-center text-[10px] pb-2 border-b border-border-subtle">
            <div>
              <span class="text-text-muted block">Pointer i (L)</span>
              <span class="text-xs font-bold text-accent-cyan">{{ leftPointerIdx !== null ? leftPointerIdx : '—' }}</span>
            </div>
            <div>
              <span class="text-text-muted block">Pointer j (R)</span>
              <span class="text-xs font-bold text-accent-purple">{{ rightPointerIdx !== null ? rightPointerIdx : '—' }}</span>
            </div>
            <div>
              <span class="text-text-muted block">Ghi đè k</span>
              <span class="text-xs font-bold text-accent-yellow">{{ kIndex !== null ? kIndex : '—' }}</span>
            </div>
          </div>

          
          <div class="flex justify-between items-center text-xs pt-1.5">
            <span class="text-text-muted">So sánh hiện tại:</span>
          ...
          <div class="flex items-center gap-2 mt-1">
            ...
            <span v-else-if="kIndex !== null" class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-accent-yellow/10 text-accent-yellow border border-accent-yellow/20 animate-pulse">
              SAO CHÉP / GHI ĐÈ
            </span>
        ...
        <p class="text-[10px] text-text-secondary leading-relaxed">
          ...
          <div class="grid grid-cols-2 gap-1.5 mt-1 text-[10px]">
            <div class="p-1.5 rounded merge-cell">
              <span class="text-text-muted block text-[9px] font-bold">Thời gian (tất cả TH)</span>
              <span class="text-accent-green font-bold">O(N log N)</span>
            </div>
            <div class="p-1.5 rounded merge-cell">
              <span class="text-text-muted block text-[9px] font-bold">Bộ nhớ bổ sung</span>
              <span class="text-accent-yellow font-bold">O(N)</span>
            </div>
        </div>
      </div>
        ...
        <h4 class="text-accent-green font-bold tracking-normal text-[10px] flex items-center gap-1">
          ...
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px]">
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded border border-accent-yellow bg-accent-yellow/20 shadow-[0_0_8px_rgba(245,158,11,0.3)] animate-pulse"></span>
...
function getLeftElementClass(idx: number) {
  if (!props.frame) return 'border-border-subtle text-text-muted';
  if (isLeftPointer(idx)) {
    return 'border-accent-cyan bg-accent-cyan/40 text-accent-cyan shadow-[0_0_8px_rgba(34,211,238,0.25)] scale-105';
  }
  ...
  return 'border-border-subtle bg-bg-secondary/20 text-text-muted';
}

function getRightElementClass(idx: number) {
  if (!props.frame) return 'border-border-subtle text-text-muted';
  if (isRightPointer(idx)) {
    return 'border-accent-purple bg-accent-purple/40 text-accent-purple shadow-[0_0_8px_rgba(192,132,252,0.25)] scale-105';
  }
  const isSorted = props.frame.sortedIndices.includes(idx);
  if (isSorted) {
    return 'border-accent-green/30 bg-accent-green/10 text-accent-green/80';
