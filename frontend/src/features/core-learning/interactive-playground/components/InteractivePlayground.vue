<template>
  <div class="playground-root">
    ...
    <div class="playground-header-bar flex flex-col lg:flex-row justify-between items-center gap-3 p-3 bg-bg-secondary/45 border-b border-border-default backdrop-blur-md z-[1001]">
      
      <div class="flex bg-bg-surface p-1 rounded-xl border border-border-default">
        <button
          @click="store.setAlgorithmMode(false)"
          :class="['px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5', !store.isAlgorithmMode ? 'bg-accent-emerald text-text-primary shadow-[0_0_12px_rgba(16,185,129,0.3)]' : 'text-text-muted hover:text-text-primary']"
        >
        ...
        <button
          ...
          :class="['px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5', store.isAlgorithmMode ? 'bg-accent-cyan text-text-primary shadow-[0_0_12px_rgba(6,182,212,0.3)]' : 'text-text-muted hover:text-text-primary']"
      ...
      <div v-if="!store.isAlgorithmMode" class="flex flex-wrap items-center justify-center gap-2">
        <div class="flex bg-bg-surface p-1 rounded-xl border border-border-default">
          <button
            ...
            :class="['px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all cursor-pointer flex items-center gap-1', store.mode === tool.mode ? 'bg-accent-emerald text-text-primary shadow-[0_0_8px_rgba(16,185,129,0.35)]' : 'text-text-muted hover:text-text-primary']"
            :title="tool.title"
          >
            <span>{{ tool.label }}</span>
          </button>
        </div>
        <div class="h-6 w-px bg-bg-surface mx-1 hidden sm:block"></div>
        <div class="text-[11px] font-mono text-text-muted hidden sm:block select-none bg-bg-surface px-2.5 py-1.5 rounded-lg border border-border-default">
      ...
      <div v-else class="flex flex-wrap items-center justify-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <select
            ...
            class="bg-bg-surface border border-border-default rounded-lg px-2.5 py-1.5 text-xs text-text-primary font-bold outline-none cursor-pointer hover:border-border-strong transition-all"
          >
        ...
        <div class="flex items-center gap-2">
          ...
          <select
            ...
            class="bg-bg-surface border border-border-default rounded-lg px-2.5 py-1.5 text-xs text-text-primary font-bold outline-none cursor-pointer hover:border-border-strong transition-all font-mono"
          >
      ...
      <div v-if="!store.isAlgorithmMode" class="flex items-center gap-1.5">
        <button
          ...
          :class="['p-2 rounded-lg border cursor-pointer transition-all duration-200', store.isPhysicsEnabled ? 'bg-accent-emerald/15 border-accent-emerald/30 text-accent-emerald shadow-[0_0_8px_rgba(16,185,129,0.2)]' : 'bg-bg-surface border-border-default text-text-muted hover:text-text-primary hover:border-border-default']"
        ...
        <button
          ...
          class="p-2 rounded-lg bg-bg-surface border border-border-default text-accent-red hover:bg-accent-red/10 hover:border-accent-red/20 cursor-pointer transition-all duration-200"
        ...
        <div class="w-px h-5 bg-bg-surface mx-0.5"></div>

        <label
          data-tour-id="graph-import-label"
          class="px-2.5 py-1.5 rounded-lg bg-bg-surface border border-border-default text-text-secondary hover:text-text-primary hover:bg-bg-surface hover:border-border-default text-xs font-semibold cursor-pointer transition-all flex items-center gap-1 select-none"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <button
          ...
          class="px-2.5 py-1.5 rounded-lg bg-bg-surface border border-border-default text-text-secondary hover:text-text-primary hover:bg-bg-surface hover:border-border-default text-xs font-semibold cursor-pointer transition-all flex items-center gap-1"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div class="relative flex-1 min-h-[200px] overflow-hidden" ref="canvasAreaRef" data-tour-id="graph-canvas">
      ...
      <div v-if="store.nodes.length === 0 && !store.isAlgorithmMode && !store.isGuideDismissed" class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10 p-6 text-center">
        <div class="max-w-md bg-bg-secondary/90 backdrop-blur-lg border border-border-strong p-6 rounded-2xl shadow-2xl transition-all select-none pointer-events-auto">
          ...
          <div class="text-left space-y-2.5 text-xs text-text-secondary pl-2">
            <div class="flex items-start gap-2">
              <span class="px-1.5 py-0.5 rounded bg-bg-surface border border-border-default font-bold text-accent-emerald">1</span>
              <span>Chọn <strong>+ Đỉnh (N)</strong> ở thanh trên và nhấp chuột bất kỳ đâu để tạo đỉnh.</span>
            </div>
            <div class="flex items-start gap-2">
              <span class="px-1.5 py-0.5 rounded bg-bg-surface border border-border-default font-bold text-accent-emerald">2</span>
              <span>Chọn <strong>↔ Cạnh (E)</strong>, nhấp giữ từ một đỉnh và kéo thả tới đỉnh kề để nối.</span>
            </div>
            <div class="flex items-start gap-2">
              <span class="px-1.5 py-0.5 rounded bg-bg-surface border border-border-default font-bold text-accent-emerald">3</span>
      ...
      <div v-if="store.isAlgorithmMode && animStore.currentFrame" class="absolute top-4 right-4 w-[360px] z-[1002] bg-bg-secondary/85 backdrop-blur-md border border-border-default rounded-xl shadow-2xl p-4 flex flex-col gap-3 transition-all duration-300">
        ...
        <div class="font-mono text-[11px] leading-relaxed text-text-secondary select-none overflow-y-auto max-h-[400px] border border-border-default rounded-lg p-2 bg-black/20">
          <div
        ...
        <div class="h-px bg-bg-surface my-0.5"></div>
    ...
    <div v-if="store.isAlgorithmMode" class="w-full bg-bg-secondary/85 backdrop-blur-md border-t border-border-default p-3.5 z-[1003] shadow-2xl">
...
import { GraphAlgorithmSimulator } from '@/features/core-learning/interactive-playground/services/GraphAlgorithmSimulator';
