  ...
  <div class="merge-panel border rounded-2xl p-4 backdrop-blur-md flex flex-col gap-3 h-full">
    <div class="flex items-center justify-between border-b border-border-default pb-2 shrink-0">
      ...
      <span 
        ...
        :class="activeSub ? 'bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20 animate-pulse' : 'bg-bg-hover/10 text-text-secondary border border-border-default'"
      >
    ...
    <div v-if="activeSub && frame" class="flex-1 flex flex-col gap-4 justify-between">
      <div class="flex flex-col gap-3 shrink-0">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1.5 p-2 rounded-xl border border-accent-cyan/15 bg-accent-cyan/5">
            <span class="text-[9px] text-accent-cyan font-bold tracking-normal">Mảng trái L [{{ activeSub.start }}..{{ midIndex }}]</span>
            <div class="flex flex-wrap gap-1">
              ...
              >
                ...
                <span v-if="isLeftPointer(activeSub.start + idx - 1)" class="text-[8px] text-accent-cyan ml-0.5 font-bold">i</span>
              </span>
            </div>
          </div>

          
          <div class="flex flex-col gap-1.5 p-2 rounded-xl border border-accent-purple/15 bg-accent-purple/5">
            <span class="text-[9px] text-accent-purple font-bold tracking-normal">Mảng phải R [{{ midIndex + 1 }}..{{ activeSub.end }}]</span>
            <div class="flex flex-wrap gap-1">
              <span
              ...
              >
                ...
                <span v-if="isRightPointer(midIndex + idx)" class="text-[8px] text-accent-purple ml-0.5 font-bold">j</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="flex-1 flex flex-col justify-between p-3 rounded-xl merge-inner-block min-h-0 text-xs">
        <div class="flex flex-col gap-2">
          ...
          <div class="grid grid-cols-3 gap-2 text-center text-[10px] pb-2 border-b border-border-default">
            <div>
              <span class="text-text-secondary block">Pointer i (L)</span>
              <span class="text-xs font-bold text-accent-cyan">{{ leftPointerIdx !== null ? leftPointerIdx : '—' }}</span>
            </div>
            <div>
              <span class="text-text-secondary block">Pointer j (R)</span>
              <span class="text-xs font-bold text-accent-purple">{{ rightPointerIdx !== null ? rightPointerIdx : '—' }}</span>
            </div>
            <div>
              <span class="text-text-secondary block">Ghi đè k</span>
              <span class="text-xs font-bold text-accent-warm">{{ kIndex !== null ? kIndex : '—' }}</span>
            </div>
          </div>

          
          <div class="flex justify-between items-center text-xs pt-1.5">
            <span class="text-text-secondary">So sánh hiện tại:</span>
            <span v-if="leftPointerIdx !== null && rightPointerIdx !== null" class="font-bold text-text-primary">
          ...
          <div class="flex items-center gap-2 mt-1">
            ...
            <span v-else-if="kIndex !== null" class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-accent-warm/10 text-accent-warm border border-accent-warm/20 animate-pulse">
              SAO CHÉP / GHI ĐÈ
            </span>
        ...
        <p class="text-[10px] text-text-secondary leading-relaxed">
          ...
          <div class="grid grid-cols-2 gap-1.5 mt-1 text-[10px]">
            <div class="p-1.5 rounded merge-cell">
              <span class="text-text-secondary block text-[9px] font-bold">Thời gian (tất cả TH)</span>
              <span class="text-accent-green font-bold">O(N log N)</span>
            </div>
            <div class="p-1.5 rounded merge-cell">
              <span class="text-text-secondary block text-[9px] font-bold">Bộ nhớ bổ sung</span>
              <span class="text-accent-warm font-bold">O(N)</span>
            </div>
        </div>
      </div>
        ...
        <h4 class="text-accent-green font-bold tracking-normal text-[10px] flex items-center gap-1">
          ...
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px]">
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded border border-accent-warm bg-accent-warm/20 shadow-[0_0_8px_rgba(245,158,11,0.3)] animate-pulse"></span>
              <span class="text-text-secondary">So sánh L và R</span>
            </div>
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded border border-accent-red bg-accent-red/20 shadow-[0_0_8px_rgba(244,63,94,0.3)]"></span>
              <span class="text-text-secondary">Ghi đè / Gộp gốc</span>
            </div>
            <div class="flex items-center gap-1.5">
            ...
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded border border-accent-cyan bg-accent-cyan/20 shadow-[0_0_8px_rgba(6,182,212,0.3)]"></span>
              <span class="text-text-secondary">Phân đoạn đệ quy</span>
            </div>
        </div>
      </div>
    </div>
...
function getLeftElementClass(idx: number) {
  if (!props.frame) return 'border-border-default text-text-muted';
  if (isLeftPointer(idx)) {
    return 'border-cyan-400 bg-cyan-950/40 text-accent-cyan shadow-[0_0_8px_rgba(34,211,238,0.25)] scale-105';
  }
  const isSorted = props.frame.sortedIndices.includes(idx);
  if (isSorted) {
    return 'border-accent-green/30 bg-accent-green/10 text-accent-green/80';
  }
  return 'border-border-default bg-bg-primary/20 text-text-secondary';
}

function getRightElementClass(idx: number) {
  if (!props.frame) return 'border-border-default text-text-muted';
  if (isRightPointer(idx)) {
    return 'border-purple-400 bg-purple-950/40 text-accent-purple shadow-[0_0_8px_rgba(192,132,252,0.25)] scale-105';
  }
  const isSorted = props.frame.sortedIndices.includes(idx);
  if (isSorted) {
    return 'border-accent-green/30 bg-accent-green/10 text-accent-green/80';
