<template>
  <div class="heap-array-container w-full flex items-center justify-between gap-2 py-1 shrink-0 select-none border-t border-border-default font-sans">
    <div class="text-[10px] font-mono text-text-secondary shrink-0">
      Heap Size: <span class="font-bold text-accent">{{ currentHeapSize }}/{{ n }}</span>
    </div>
    ...
    <div class="relative flex-1 flex items-center justify-center overflow-x-auto min-w-0">
      ...
      >
        ...
        >
          ...
          <span class="font-mono text-[9px] text-text-secondary">[{{ idx }}]</span>
        </div>
      </transition-group>
    </div>
  </div>
