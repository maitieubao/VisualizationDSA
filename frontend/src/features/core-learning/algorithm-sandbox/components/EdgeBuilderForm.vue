<template>
  <div class="edge-builder-form grid grid-cols-12 gap-3 min-h-0">
    ...
    <div class="col-span-5 form-container p-3 rounded-xl border border-border-default bg-black/10 flex flex-col gap-2">
    ...
    <div class="col-span-7 edges-list-container flex flex-col gap-2 min-h-0">
      ...
      <div v-if="edges.length === 0" class="text-[10px] text-text-muted italic text-center py-6 border border-dashed border-border-default rounded-xl flex-1 flex items-center justify-center">
        Chưa có cạnh.
      </div>
          ...
          :class="[
            ...
            isSelected(edge.id)
              ...
              : isHovered(edge.id)
                ...
                : 'bg-bg-surface border-border-default hover:bg-bg-surface hover:border-border-default'
          ]"
        >
          <div class="flex items-center gap-1 font-mono text-[11px]">
            <span class="font-bold text-text-primary">{{ getNodeLabel(edge.from) }}</span>
            <span class="text-text-muted"><BaseIcon name="arrow-right" class="w-2.5 h-2.5 inline-block align-text-bottom" /></span>
            <span class="font-bold text-text-primary">{{ getNodeLabel(edge.to) }}</span>
            <span class="px-1 py-0.2 bg-bg-surface border border-border-default text-accent-yellow rounded font-semibold text-[9px] ml-1">
              w={{ edge.weight }}
            </span>
          </div>
