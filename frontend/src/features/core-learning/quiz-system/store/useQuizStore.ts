...
import type { QuizQuestion, QuizCheckpoint, CanvasNodeDTO } from '@/features/core-learning/quiz-system/types/quiz.types';
...
export const useQuizStore = defineStore('quizSystem', () => {
  ...
  const completedQuizAnswers = ref<Array<{ questionId: string; selectedIndex: number }>>([]);
  ...
  function submitOptionAnswer(index: number): void {
    ...
    completedQuizAnswers.value.push({ questionId: activeQuestion.value.id, selectedIndex: index });
  }
  ...
  function handleCanvasClickAnswer(clickX: number, clickY: number, nodes: CanvasNodeDTO[]): void {
    ...
    completedQuizAnswers.value.push({ questionId: activeQuestion.value.id, selectedIndex: result.matchedNodeId ? nodes.findIndex(n => n.id === result.matchedNodeId) : -1 });
...
async function syncSessionToServer(): Promise<void> {
  ...
  if (!passed) return;
  const xpEarned = Math.round((sessionCorrect.value / sessionTotal.value) * 50);
  ...
  return {
    ...
    sessionCorrect, sessionTotal, completedQuizAnswers,
