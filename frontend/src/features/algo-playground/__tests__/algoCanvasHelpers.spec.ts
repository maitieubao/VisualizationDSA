...
describe('algoCanvasHelpers — drawPlaybackFrame (AL-009)', () => {
  ...
  it('graph: node visited → màu visited, node không visited → default', () => {
    ...
    expect(fillStyles).not.toContain(COLORS.nodeActive);
  });
...
describe('algoCanvasHelpers — drawPlaybackFrameTransition (AL-009): lerp màu', () => {
  it('tree: node active→visited lerp giữa 2 frame (t=0 / 0.5 / 1)', () => {
    ...
    for (const [t, expected] of [
      [0, 'rgb(245,158,11)'],   // 100% màu prev (active amber #f59e0b)
      [1, 'rgb(16,185,129)'],   // 100% màu curr (visited green #10b981)
    ...
    // Midpoint: nội suy chính xác (245→16)/2 = 130.5 → 131, (158→185)/2 = 171.5 → 172, (11→129)/2 = 70
    const { ctx, calls } = makeCtx();
    drawPlaybackFrameTransition(ctx as unknown as CanvasRenderingContext2D, W, H, prev, curr, 0.5);
    const fillStyles = calls.fillStyle ?? [];
    expect(fillStyles).toContain('rgb(131,172,70)');
  });
  ...
  it('tree: cạnh pruned→normal lerp màu giữa 2 frame', () => {
    ...
    // Từ nodePruned rgba(51,65,85,0.25) → edgeDefault #475569:
    // r = round((51+71)/2)=61, g = round((65+85)/2)=75, b = round((85+105)/2)=95
    // a = 0.25 + (1-0.25)*0.5 = 0.625 → toFixed(2) = "0.63" (JS round-half-up)
    const strokeStyles = calls.strokeStyle ?? [];
    expect(strokeStyles).toContain('rgba(61,75,95,0.63)');
  });
  ...
  it('graph: cạnh highlight lerp từ default → highlight giữa 2 frame', () => {
    ...
    // #475569 → #fbbf24: r=round(161)=161, g=round((85+191)/2)=138, b=round((105+36)/2)=70.5→71
    const strokeStyles = calls.strokeStyle ?? [];
    expect(strokeStyles).toContain('rgb(161,138,71)');
  });
  ...
  it('graph: node visited→active lerp màu giữa 2 frame', () => {
    ...
    // green #10b981 → amber #f59e0b: r=round(130.5)=131, g=round(171.5)=172, b=70
    const fillStyles = calls.fillStyle ?? [];
    expect(fillStyles).toContain('rgb(131,172,70)');
  });
