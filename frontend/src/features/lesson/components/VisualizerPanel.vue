<template>
  <div class="lesson-step-viz flex flex-col h-full w-full bg-bg-primary relative overflow-hidden">
    ...
    <button
      ...
      class="absolute top-3 right-4 z-30 px-3.5 py-1.5 bg-accent/90 hover:bg-accent text-text-primary rounded-xl text-xs font-bold transition-all shadow-lg backdrop-blur-md cursor-pointer border border-border-accent/30 flex items-center gap-1.5 animate-pulse"
    >
    ...
    <div
      v-else
      class="absolute top-3 right-4 z-30 px-3.5 py-1.5 bg-bg-hover/90 text-text-secondary rounded-xl text-xs font-bold shadow-lg backdrop-blur-md border border-border-default flex items-center gap-1.5"
    >
      <svg class="w-4 h-4 animate-spin text-text-secondary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div class="flex-1 min-h-0 relative w-full h-full">
      <template v-if="visualizerComponent">
        ...
        <button
          ...
          :class="isLoadingLecture ? 'bg-bg-surface text-text-secondary border-border-default' : 'bg-accent-cyan/90 hover:bg-accent-cyan text-text-primary border-accent-cyan/30'"
        >
...
import { useLectureStore, loadLecture, LectureOverlay } from '@/features/e-lecture';
