  ...
  <div class="lesson-step-codelab flex flex-col lg:flex-row h-full w-full bg-bg-secondary overflow-hidden text-text-primary font-sans">
    ...
    <div class="w-full lg:w-1/2 h-full flex flex-col border-r border-border-subtle bg-bg-secondary overflow-hidden">
      ...
      <div v-show="activeTab === 'problem'" class="flex-1 overflow-y-auto p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-border-subtle pb-3">
          ...
          <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-accent-yellow/80 text-accent-yellow border border-accent-yellow/30">
            Trung bình (Medium)
          </span>
        </div>
        ...
        <div class="text-xs leading-relaxed text-text-secondary space-y-3">
          ...
          <div v-if="task?.testCases?.[0]" class="p-3 rounded-xl bg-bg-secondary border border-border-subtle space-y-1">
            <span class="text-[10px] font-bold text-text-muted uppercase font-mono">Ví dụ 1:</span>
      ...
      <div v-show="activeTab === 'testcases'" class="flex-1 overflow-y-auto p-5 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2 group relative">
            <h3 class="text-xs font-bold uppercase text-text-muted">Bộ Kiểm Thử (Testcases)</h3>
            ...
            <div class="absolute left-0 top-5 w-48 p-2 bg-bg-surface text-xs text-text-secondary rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none border border-border-subtle">
        ...
        <div v-if="isRunning && testResults.length === 0" class="space-y-3">
          <div v-for="i in task?.testCases?.length || 2" :key="i" class="h-20 rounded-xl bg-bg-surface/50 animate-pulse border border-border-subtle"></div>
        </div>

        <div v-for="(tc, idx) in task?.testCases" :key="idx" class="p-3.5 rounded-xl bg-bg-secondary border border-border-subtle space-y-2 text-xs">
          <div class="flex items-center justify-between">
      ...
      <div v-show="activeTab === 'solution'" class="flex-1 overflow-y-auto p-5 space-y-3">
        <h3 class="text-xs font-bold uppercase text-text-muted">Giải pháp tham khảo</h3>
        <div class="p-4 rounded-xl bg-bg-secondary border border-border-subtle space-y-2">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1.5 text-xs font-bold text-accent-yellow">
            ...
            <button @click="showSolution = !showSolution" class="text-[11px] text-text-muted hover:text-white cursor-pointer">
    ...
    <div class="w-full lg:w-1/2 h-full flex flex-col bg-bg-secondary">
      ...
      <div class="flex-1 min-h-0 relative p-3">
        ...
        <textarea
          v-model="userCode"
          class="w-full h-full bg-bg-secondary text-accent font-mono text-xs p-4 rounded-xl border border-border-subtle focus:outline-none focus:border-accent/50 resize-none leading-relaxed"
          spellcheck="false"
        ></textarea>
      </div>
      ...
      <div class="p-4 border-t border-border-subtle bg-bg-secondary flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2">
          <button
            ...
            :disabled="isRunning"
            class="px-4 py-2 bg-bg-surface hover:bg-bg-hover text-text-primary rounded-xl text-xs font-bold transition-all border border-border-subtle disabled:opacity-50 cursor-pointer flex items-center gap-2"
          >
