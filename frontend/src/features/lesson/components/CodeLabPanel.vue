    ...
    <div class="w-full lg:w-1/2 h-full flex flex-col border-r border-border-default bg-bg-secondary/60 overflow-hidden">
      ...
      <div v-show="activeTab === 'problem'" class="flex-1 overflow-y-auto p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-border-default pb-3">
          ...
          <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-accent-warm/20 text-accent-warm border border-accent-warm/30">
            Trung bình (Medium)
          </span>
        </div>
        ...
        <div class="text-xs leading-relaxed text-text-secondary space-y-3">
          ...
          <div v-if="task?.testCases?.[0]" class="p-3 rounded-xl bg-bg-primary border border-border-default space-y-1">
            <span class="text-[10px] font-bold text-text-secondary uppercase font-mono">Ví dụ 1:</span>
            <div class="font-mono text-xs text-accent">Input: {{ task.testCases[0].input }}</div>
            <div class="font-mono text-xs text-accent-green">Output: {{ task.testCases[0].expectedOutput }}</div>
          </div>
        </div>
      </div>
      ...
      <div v-show="activeTab === 'testcases'" class="flex-1 overflow-y-auto p-5 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2 group relative">
            <h3 class="text-xs font-bold uppercase text-text-secondary">Bộ Kiểm Thử (Testcases)</h3>
            <svg class="w-3.5 h-3.5 text-text-muted hover:text-text-secondary cursor-help" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div class="absolute left-0 top-5 w-48 p-2 bg-bg-hover text-xs text-text-secondary rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none border border-border-default">
              Testcase ẩn sẽ tự động được chạy khi bạn bấm nút <b class="text-accent-green">Nộp Code</b>.
            </div>
          </div>
          <span v-if="isRunning" class="text-xs text-accent flex items-center gap-2">
        ...
        <div v-if="isRunning && testResults.length === 0" class="space-y-3">
          <div v-for="i in task?.testCases?.length || 2" :key="i" class="h-20 rounded-xl bg-bg-surface animate-pulse border border-border-default"></div>
        </div>

        <div v-for="(tc, idx) in task?.testCases" :key="idx" class="p-3.5 rounded-xl bg-bg-primary border border-border-default space-y-2 text-xs">
          <div class="flex items-center justify-between">
            <span class="font-bold text-text-secondary">Testcase #{{ idx + 1 }} {{ tc.isHidden ? '(Ẩn)' : '' }}</span>
            <span v-if="testResults[idx]" :class="testResults[idx].passed ? 'text-accent-green' : 'text-accent-red'" class="font-bold text-[11px] flex items-center gap-1">
              <BaseIcon v-if="testResults[idx].passed" name="check" class="w-3 h-3" />
          ...
          <div v-if="!tc.isHidden" class="font-mono text-[11px] text-text-secondary">
            ...
            <div v-if="testResults[idx] && !testResults[idx].passed">Actual: <span class="text-accent-red">{{ testResults[idx].actualOutput }}</span></div>
          </div>
      ...
      <div v-show="activeTab === 'solution'" class="flex-1 overflow-y-auto p-5 space-y-3">
        <h3 class="text-xs font-bold uppercase text-text-secondary">Giải pháp tham khảo</h3>
        <div class="p-4 rounded-xl bg-bg-primary border border-border-default space-y-2">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1.5 text-xs font-bold text-accent-warm">
              <svg class="w-4 h-4 text-accent-warm" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              <span>Mã nguồn chuẩn</span>
            </div>
            <button @click="showSolution = !showSolution" class="text-[11px] text-text-secondary hover:text-text-primary cursor-pointer">
              {{ showSolution ? 'Ẩn' : 'Xem (-20 XP)' }}
            </button>
          </div>
          <pre v-if="showSolution" class="text-xs text-text-secondary leading-relaxed font-mono bg-bg-secondary p-3 rounded-lg overflow-x-auto">{{ task?.solution }}</pre>
        </div>
      </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 h-full flex flex-col bg-bg-primary">
      ...
      <div class="px-4 py-2.5 border-b border-border-default bg-bg-secondary/80 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10px] px-2 py-0.5 rounded bg-bg-hover text-text-secondary font-mono">JavaScript</span>
        </div>
      ...
      <div class="flex-1 min-h-0 relative p-3">
        ...
        <textarea
          v-model="userCode"
          class="w-full h-full bg-bg-primary text-accent-light font-mono text-xs p-4 rounded-xl border border-border-default focus:outline-none focus:border-border-accent/50 resize-none leading-relaxed"
          spellcheck="false"
        ></textarea>
      </div>
      ...
      <div class="p-4 border-t border-border-default bg-bg-secondary/90 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2">
          <button
            ...
            :disabled="isRunning"
            class="px-4 py-2 bg-bg-hover hover:bg-bg-hover text-text-primary rounded-xl text-xs font-bold transition-all border border-border-default disabled:opacity-50 cursor-pointer flex items-center gap-2"
          >
...
import { executeInSandbox } from '@/features/core-learning/code-to-visualization/engine/WorkerLifecycleCoordinator';
