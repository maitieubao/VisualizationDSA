...
- **V?n d?:** To�n b? 21 thu?t to�n c?a Algo Playground render qua monolith `SortingAnimationEngine.ts` (1138 d�ng) + `algoCanvasHelpers.ts` (1057 d�ng); 12/21 thu?t to�n (search, two-pointers, stack/queue, tree, graph) d�ng chung pipeline generic kh�ng m� t? d�ng t�nh ch?t (stack/queue ch? l� badge ch?, BST kh�ng t� node t�m th?y, pointer search nh?y c�c, overlay bi?n m?t khi transition, merge/counting kh�ng lerp).
- **Quy?t d?nh:**
  1. T�ch render layer theo nh�m thu?t to�n: `engine/renderers/` g?m 11 file renderer (ArraySorting, MergeSort, HeapSort, CountingSort, RadixSort, BucketSort, Searching, TwoPointers, StackQueue, Tree, Graph) c�ng interface `AlgoRenderer` (`types.ts`).
  2. L�i engine duy nh?t `AlgoAnimationEngine.ts` ch? gi? rAF loop + snapshot + transition detection + dispatch qua `rendererRegistry.ts` (map algorithmId ? renderer, uu ti�n data-driven theo snapshot: mergeState/heapState).
  3. T�ch `algoCanvasHelpers.ts` ? 9 file nh? trong `renderer/` (colors, geometry, arrayBars, nodeStates, treeDrawer, graphDrawer, overlays, playbackFrame, barSortTransitions).
  4. Fix animation theo nh�m: merge chip bay L/R?OUT, counting ghost bay input?count?output, pointer search lerp + v�ng dim, two-pointers c?a s? highlight, stack/queue visual LIFO/FIFO th?t v?i push/pop animation, BST found node glow (d�ng searchFound + activeIds v� demo g?i `found(-1)`), overlay v? cu?i M?I nh�nh transition.
  5. Demo fix: bucket-sort sort phase d�ng `setBucketComparing` (insertion sort tr?c quan); quick-sort push/pop partition stack hi?n th? ngan x?p tu?ng minh.
- **H? qu?:** Th�m nh�m thu?t to�n m?i = 1 file renderer + 1 d�ng registry (Open-Closed � AGENTS.md Quy t?c 1). M?i nh�m m� t? d�ng t�nh ch?t thu?t to�n v?i animation mu?t. X�a 3 file engine monolith cu.
